...
#### 使用 Django 的分页器（ Paginator ）对数据进行分页。

例如产品列表页，查询数据库后分页，只把当前页的数据传递给模板



（1）在 xiaomijiaju/models.py 里定义模型：
...
（2）在 xiaomijiaju/views.py 里查询产品并传递给模板：
...
（3）在 product_list.html 里用 for 循环渲染：
...
products = [
    {'name': '小米厨房家具橱柜', 'image': 'img/chug.jpg', 'price': 2499.00, 'category': '储物'},
    {'name': '小米厨房家具岛台', 'image': 'img/dt.jpg', 'price': 3499.00, 'category': '桌台'},
    # ... ...
    {'name': '小米厨房家具洗碗机', 'image': 'img/xwj.jpg', 'price': 2999.00, 'category': '电器'}
]
